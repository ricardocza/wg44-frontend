...
interface iGraphContext {
  ...
  dailyMean: iMean;
  setDailyMean: React.Dispatch<React.SetStateAction<iMean>>;
  serverStatus: string;
...
export const GraphProvider = ({ children }: iGraphProviderProps) => {
  ...
  const [dailyMean, setDailyMean] = useState({} as iMean);
  ...
  const requestCurrentClose = async () => {
    try {
      const tickerResponse = await api.get(
        "/api/market/new/database/list/current-close"
      );
  ...
  const requestGraphData = async () => {
    try {
      const response = await api.get(
        `/api/market/new/database/${selectedAsset}/list`
      );
      return response;
    } catch (error: any) {
      console.log(error);
  ...
  const requestMeanData = async (
    ...
    try {
      const meanResponse = await api.get(
        `/api/market/new/database/${selectedAsset}/list/mean/?start_date=${start_date}&end_date=${end_date}`
      );
    ...
    if (graphData[0]?.id) {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        const minute: number = parseInt(
          labels[labels.length - 1].split(":")[1]
        );
        const second: number = parseInt(
          labels[labels.length - 1].split(":")[2]
        );
  ...
  const updateTicker = async () => {
    ...
    if (currentData === false || globalMean === false) {
      ...
      const data = currentData.data.results.sort(
        (a: iGraphData, b: iGraphData) => (b.id < a.id ? 1 : -1)
      );
